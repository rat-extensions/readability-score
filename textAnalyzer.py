...
class TextAnalyzer:
    ...
    def analyzeDe(self, main_text):
        ...
        easy_threshold = 70  # You can adjust this threshold
        hard_threshold = 30  # You can adjust this threshold
        
            # Function to print scores with color and emojis based on their readability level
        def print_colored_score(score, label):
            if score >= easy_threshold:
                emoji_icon = emoji.emojize(":grinning_face_with_smiling_eyes:")  # Emoji for easy
                color_code = "\033[92m"  # Green
            elif score >= hard_threshold:
                emoji_icon = emoji.emojize(":neutral_face:")  # Emoji for average
                color_code = "\033[93m"  # Yellow
            else:
                emoji_icon = emoji.emojize(":disappointed_face:")  # Emoji for hard
                color_code = "\033[91m"  # Red
            print(f"{label} {emoji_icon}: {color_code}{score}\033[0m")
    
        # Print readability scores with color and emojis
        print_colored_score(flesch_reading_ease, "Flesch Reading Ease Score")
        
            # Function to print grade levels with color and emojis based on their readability level
        def print_colored_grade_level(grade, label):
            if grade <= 6:
                emoji_icon = emoji.emojize(":grinning_face_with_smiling_eyes:")  # Emoji for easy
                color_code = "\033[92m"  # Green
        ...
        # Print grade levels with color and emojis
        print_colored_grade_level(flesch_kincaid_grade, "Flesch-Kincaid Grade")
    
        # Print Wiener Sachtextformel scores
